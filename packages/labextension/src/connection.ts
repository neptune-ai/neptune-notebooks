...
export class NeptuneConnection {
    ...
    updateParams = (params: Partial<NeptuneConnectionParams>) => {
        let {apiToken, project, notebookId} = this.params;
        if (params.apiToken) {
        ...
        let newParams = {apiToken: apiToken, project: project, notebookId: notebookId};
    ...
    listProjects = () => {
        return this.getAuthorizationHeader().then(authorizationHeader => {
            return fetch(this.getApiAddress() + "/api/backend/v1/projects2?userRelation=memberOrHigher",
                {
                    headers: {
    ...
    getNotebook = () => {
        return this.getAuthorizationHeader().then(authorizationHeader => {
            return fetch(this.getApiAddress() + "/api/leaderboard/v1/notebooks/" + this.params.notebookId,
                {
                    headers: {
                        "Authorization": authorizationHeader
                    }
...
export function createConnection(params: Partial<NeptuneConnectionParams>) {
    ...
    if (!params.project && params.notebookId) {
        return connection.getNotebook()
            .then(notebook =>
                connection.listProjects().then(projects => {
